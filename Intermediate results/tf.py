...
for q in range(1, 226):
	...
	#creating tf_matrix
	tf_vectorizer = StemmedTfidfVectorizer(use_idf=False, stop_words='english', analyzer='word', ngram_range=(1,1))
	...
	csim = np.array(cosine_similarity(tf_matrix[0], tf_matrix[1:len(corpus)])[0])
	#using 10 feedback documents
	x=10

	arr=np.array([0])
	arr1=np.append(arr,csim.argsort()[-x:][::-1])
	arr2=(tf_matrix.todense())[arr1]
	...
	query_vector= np.squeeze(np.asarray(tf_matrix.todense()[0]))
	...
	tf_matrix[0]=sparse.csr_matrix(estimated_query_vector)

	#cosine similarity of documents with queue
	csim1 = np.array(cosine_similarity(tf_matrix[0], tf_matrix[1:len(corpus)])[0])
